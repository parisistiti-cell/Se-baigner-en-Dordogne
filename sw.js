...
// IMPORTANT : incrémenter ce numéro à chaque déploiement qui change le
// contenu de l'app shell (JS, JSON de données, icônes...). C'est ce qui
...
const CACHE_VERSION = 'tp-baignades-v3';
...
// Fichiers de données versionnées qui doivent TOUJOURS être revérifiés sur le
// réseau en priorité (réseau d'abord), plutôt que servis depuis le cache sans
// vérification comme le reste de l'app shell. Ajoutez ici tout autre fichier
// .json de données amené à changer régulièrement.
const DONNEES_RESEAU_PRIORITAIRE = [
    'Last10mois.json'
];

function estUneDonneeAMettreAJour(url) {
    return DONNEES_RESEAU_PRIORITAIRE.some((nom) => url.pathname.endsWith(nom));
}
...
self.addEventListener('fetch', (event) => {
    ...
    // 2bis. Données dynamiques versionnées (Last10mois.json, etc.) : réseau
    //       d'abord, avec repli sur le cache uniquement si hors-ligne. Sans
    //       cette règle, ces fichiers tombaient dans le cas générique n°3
    //       (cache-first) : une fois mis en cache une première fois, ils
    //       n'étaient plus jamais revérifiés auprès du réseau, même après un
    //       nouveau déploiement — d'où l'affichage d'une version périmée.
    if (estUneDonneeAMettreAJour(url)) {
        // "cache: 'no-store'" est CRUCIAL ici : sans lui, fetch() respecte les
        // en-têtes Cache-Control envoyés par GitHub Pages (souvent max-age=600,
        // soit 10 min) et le NAVIGATEUR répond depuis son propre cache HTTP
        // local sans même contacter le réseau — le Service Worker ne voit
        // alors jamais passer de vraie requête réseau, malgré la stratégie
        // "réseau d'abord". On ajoute en plus un paramètre anti-cache dans
        // l'URL en filet de sécurité, au cas où un intermédiaire (proxy,
        // certains navigateurs mobiles) ignorerait no-store.
        const urlSansCache = new URL(request.url);
        urlSansCache.searchParams.set('_sw', Date.now().toString());
        const requeteSansCache = new Request(urlSansCache.toString(), {
        ...
        event.respondWith(
            fetch(requeteSansCache)
                .then((reponseReseau) => {
                    if (reponseReseau && reponseReseau.ok) {
                        const copie = reponseReseau.clone();
                        // On met en cache sous la clé d'origine (sans le paramètre
                        // "_sw"), pour que le repli hors-ligne retrouve bien le fichier.
                        caches.open(APP_SHELL_CACHE).then((cache) => cache.put(request, copie));
                    }
                    return reponseReseau;
                })
